...
class Programacao extends CI_Controller {
	...
	public $nomeLista = "lista_programacao"; // nome do view  de listagem
	public $nomeModel = "programacao_model"; // nome do model
	public $nomeForm = "programacao";        // nome da view de cadastro
	public $nomeAltera = "alterar_programacao"; // nome da view de alterar
	public $temFoto = false;
	public $tabelaFoto = "fotos_programacao"; // nome da tabela de foto
	public $idFoto = "id_programacao";       // nome do campo da tabela de foto
	public $dentroDados = "programacao";     // nome que vai nos dados_
	public $nomeController = "programacao"; // nome do controlador
    ...
    function cadastrar() {
       ...
       $jaexiste = false;
        ...
        $validacoes = array(
            array('field' => 'cidade',       'label' => 'Cidade', 'rules' => 'required|min_length[1]')
        ...
        if ($this->form_validation->run() == FALSE || $jaexiste == true) {  // Valida se os campos estao preenchidos e verifica se a foto ja existe
            ...
            $this->formcad($erro); // se o erro n for da foto chama o metodo sem o parametro de erro
                   ...
                   // TIAGO TESTE
					...
					$data['data_inicio']  = data_br_to_us($this->input->post('data_inicio'));
                    $data['data_fim']  = data_br_to_us($this->input->post('data_fim'));
					$data['per_visi']  = $this->input->post('per_visi');
                    $data['local']  = $this->input->post('local');
                    $data['atividades']  = $this->input->post('atividades');
                    $data['visi_gui']  = $this->input->post('visi_gui');
                    $data['oficinas']  = $this->input->post('oficinas');
